...
public class Eq implements Expr {
	...
	public static ArrayList<String> generateEq(Table table) {
		...
		for (int i = 0; i < table.whereCols.length; i++) {
			...
			case Column.CINT:
				for (int j = 0; j < const_int.length; j++) {
					str = table.aliasName + "." + table.whereCols[i].colName
							+ "=" + const_int[j];
					list.add(str);
				}
				str = table.aliasName + "." + table.whereCols[i].colName + "="
		...
		System.out.println("generateEq "+table.tableName +" sql num="+list.size());
		return list;
	}
